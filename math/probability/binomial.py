...
class Binomial:
    ...
    def cdf(self, k):
        ...
        - Uses the pmf method to compute the CDF as the sum of PMF values from 0 to k.
        """
        # Convert k to an integer if it is not
        k = int(k)
        ...
        # If k is greater than or equal to n, return 1 (since CDF is cumulative up to n)
        if k >= self.n:
            return 1.0
