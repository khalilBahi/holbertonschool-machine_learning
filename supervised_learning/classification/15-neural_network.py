...
class NeuralNetwork:
    ...
    def train(
        ...
        if graph:
            # Ensure costs includes the last cost value after the loop
            costs.append(self.cost(Y, self.__A2))  # Appending final cost

            # Create an array for the x-axis (iterations)
            iterations_range = np.arange(0, iterations + 1, step)

            # Plot costs, ensure lengths of x and y match
            plt.plot(iterations_range, costs[: len(iterations_range)], "b")
