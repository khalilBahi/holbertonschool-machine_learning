...
def inception_network():
    ...
    """
    input_layer = K.Input(shape=(224, 224, 3), name="input_1")
    ...
    max_pool1 = K.layers.MaxPooling2D(
        pool_size=(3, 3), strides=(2, 2), padding="same", name="max_pooling2d"
    )(conv1)
    ...
    max_pool2 = K.layers.MaxPooling2D(
        ...
        padding="same", name="max_pooling2d_1"
    )(conv3)
    ...
    max_pool3 = K.layers.MaxPooling2D(
        pool_size=(3, 3), strides=(2, 2),
        padding="same", name="max_pooling2d_2"
    )(inception3b)

    inception4a = inception_block(max_pool3,
                                  [192, 96, 208, 16, 48, 64])
    ...
    inception5a = inception_block(
        max_pool4, [256, 160, 320, 32, 128, 128]
    )  # Inception 5a
    ...
    avg_pool = K.layers.AveragePooling2D(
        pool_size=(7, 7), strides=(1, 1), padding="valid", name="avg_pool"
    )(inception5b)
    dropout = K.layers.Dropout(rate=0.4, name="dropout")(avg_pool)
    flatten = K.layers.Flatten(name="flatten")(dropout)
    dense = K.layers.Dense(
        units=1000, activation="softmax", kernel_initializer=init, name="dense"
    )(flatten)

    model = K.models.Model(inputs=input_layer, outputs=dense, name="model")
    return model
